...
func (this *SmartContractManager) TransactSmartContract(
	...
	TransactionOptions := &bind.TransactOpts{} // Forming Transaction Options, based on the Smart Contract Info

	Transaction, TransactError := this.Transact(TransactionOptions,
	...
	models.Database.Model(&models.Customer{}).Where(
		"id = ?", TransactionOptions.From.Hash()).Find(&SmartContractOwner)

	// Obtaining the Purchaser of the Smart Contract
	models.Database.Model(&models.Customer{}).Where(
		"address = ?", Transaction.To().Hash()).Find(&SmartContractPurchaser)
...
func (this *SmartContractManager) CreateSmartContract(AuthCredentials bind.TransactOpts,
	...
	// Deploying Contrat
	ContractAddress, tx, Contract, ContractError := bind.DeployContract(
...
func (this *SmartContractManager) GetCustomerSmartContracts(CustomerId string) ([]models.SmartContract, []models.SmartContract, error) {

	var customer models.Customer
	var smartContract models.SmartContract
	var ownedContracts []models.SmartContract
	var purchasedContracts []models.SmartContract

	Customer := models.Database.Model(&models.Customer{}).Where(
		"id = ?", CustomerId).Preload("SmartContracts").Find(&customer)

	ownedContracts = smartContract.GetOwnedSmartContracts(fmt.Sprint(customer.ID))
	purchasedContracts = smartContract.GetPurchasedSmartContracts(fmt.Sprint(customer.ID))

	if Customer.Error != nil || errors.Is(Customer.Error, gorm.ErrRecordNotFound) {
		return nil, nil, errors.New("Customer Not Found")

	} else {
		return ownedContracts, purchasedContracts, nil
	}
}

func (this *SmartContractManager) GetCustomerSmartContract(SmartContractId string) (models.SmartContract, error) {
	var SmartContract models.SmartContract
	Transaction := models.Database.Model(&SmartContract).Where(
		"id = ?", SmartContractId).Find(&SmartContract)
	return SmartContract, Transaction.Error
}
